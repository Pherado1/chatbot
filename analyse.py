...
def setup_language_model():
    """Set up a language model with optimized settings for LLaMA2"""
    if HUGGINGFACE_API_TOKEN:
        # Use a hosted model from HuggingFace
        ...
        # Use Ollama with optimized settings for LLaMA2
        ...
        try:
            llm = OllamaLLM(
                ...
                num_thread=6,  # Adjust based on your CPU cores (leaving some free)
            )
            # Test the connection
            llm.invoke("test")
            ...
            print(
                "1. Make sure Ollama is running (run 'ollama serve' in another terminal)"
            )
...
def cached_response_generation(customer_message, best_practices, llm_id):
    ...
    response = chain.invoke(
        {"message": customer_message, "best_practices": best_practices}
    )
...
def generate_response(customer_message):
    ...
    status.text("Generating tailored response with LLaMA2...")
...
def create_streamlit_ui():
    ...
    st.write(
        "This tool generates customer responses based on similar past conversations using LLaMA2."
    )
    ...
    if not llm:
        st.error(
            "Failed to initialize language model. Please check if Ollama is running or if your HuggingFace API token is valid."
        )
        st.info("Check the console output for troubleshooting steps.")
        st.stop()
    ...
    with tab1:
        if not vectorstore:
            st.warning(
                "No vector database found. The system will attempt to create one when you generate a response."
            )
                ...
                if not st.session_state.get("vectorstore"):
                    with st.spinner("Creating vector database..."):
                        ...
                        if chunks:
                            ...
                            st.error(
                                "Failed to load documents. Please check your CSV file."
                            )
                            st.stop()
    ...
    with tab2:
        ...
        with col2:
            # Add model info
            st.metric(
                "Model", "LLaMA2" if not HUGGINGFACE_API_TOKEN else "flan-t5-large"
            )
            st.metric(
                "Vector DB Size",
                f"{os.path.getsize('faiss_index') / (1024**2):.1f} MB"
        ...
        if st.session_state.response_times:
            ...
            st.line_chart(
                pd.DataFrame({"Response Time (s)": st.session_state.response_times})
            )
        else:
            st.info(
                "No response data available yet. Generate responses to see performance metrics."
            )
